...
public class SimpleSomNeuron extends AbstractNeuron implements SomOutputNeuron {
    ...
    public void train(final double learningRate, final double neighborhoodAdjustment) {
        for (final Synapse<Neuron> source : this.brain.getInEdges(this)) {
    ...
    public void tick() {
        ...
        double activity = 0.0;
        for (final Synapse<Neuron> currentSynapse : this.brain.getInEdges(this)) {
        ...
        for (final Synapse<Neuron> current : this.brain.getTraversableEdges(this)) {
