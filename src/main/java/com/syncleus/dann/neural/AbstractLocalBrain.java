...
public abstract class AbstractLocalBrain<IN extends InputNeuron, ON extends OutputNeuron, N extends Neuron, S extends Synapse<N>> extends AbstractBidirectedAdjacencyGraph<N, S> implements Brain<IN, ON, N, S>, StrongConnectivityOptimizedGraph<N, S> {
    ...
    private final Set<N> neurons = new HashSet<>();
    private final Set<ON> outputNeurons = new HashSet<>();
    private final Set<IN> inputNeurons = new HashSet<>();
    private final Set<S> synapses = new HashSet<>();
    ...
    protected boolean add(final N newNeuron) {
        ...
        if (this.neurons.add(newNeuron)) {
            this.outMap.put(newNeuron, new HashSet<>());
            this.inMap.put(newNeuron, new HashSet<>());
    ...
    protected boolean remove(final N removeNeuron) {
        ...
        if (this.neurons.remove(removeNeuron)) {
            final Set<Synapse> removeEdges = new HashSet<>();
    ...
    public Set<S> getAdjacentEdges(final N node) {
        /* TODO lazy iterator wrapper */
        final Set<S> nodeSynapses = new HashSet<>();
        nodeSynapses.addAll(getTraversableEdges(node));
        nodeSynapses.addAll(getInEdges(node));
        return nodeSynapses.isEmpty() ? Collections.EMPTY_SET :
                Collections.unmodifiableSet(nodeSynapses);
    }

    @Override
    public Set<S> getTraversableEdges(final N node) {
        final Set<S> ee = this.outMap.get(node);
        return ee!=null ? ee : Collections.EMPTY_SET;
    }

    @Override
    public Set<S> getInEdges(final N node) {
        final Set<S> ee = this.inMap.get(node);
        return ee!=null ? ee : Collections.EMPTY_SET;
    }
    ...
    public boolean isStronglyConnected(final N leftNode, final N rightNode) {
        ...
        final Set<S> jointSet = new HashSet<>(outSet);
    ...
    public List<N> getAdjacentNodes(final N node) {
        ...
        final List<N> neighbors = new ArrayList<>();
