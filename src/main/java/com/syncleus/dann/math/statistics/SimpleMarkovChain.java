...
public class SimpleMarkovChain<S> extends AbstractMarkovChain<S> {
    ...
    public SimpleMarkovChain(final Map<List<S>, Map<S, Double>> transitionProbabilities, final int order, final Set<S> states) {
        ...
        final int columns = (Math.max(this.states.size(), transitionProbabilities.size()));
        ...
        Set<List<S>> rowHeadersLeft = new HashSet<>(transitionProbabilities.keySet());
        ...
        if (rowHeadersLeft.remove(Collections.<S>emptyList())) {
            this.rowMapping.add(Collections.emptyList());
        }
        ...
        for (final List<S> rowHeader : rowMapping) {
            final Map<S, Double> rowTransition = Collections.unmodifiableMap(new LinkedHashMap<>(transitionProbabilities.get(rowHeader)));
    ...
    private static <S> Map<List<S>, Map<S, Double>> packTransitions(final Map<S, Map<S, Double>> transitions) {
        final Map<List<S>, Map<S, Double>> pack = new LinkedHashMap<>(transitions.size());
        for (Map.Entry<S, Map<S, Double>> transitionEntry : transitions.entrySet())
            if (transitionEntry.getKey() == null)
                pack.put(Collections.emptyList(), transitionEntry.getValue());
            else
