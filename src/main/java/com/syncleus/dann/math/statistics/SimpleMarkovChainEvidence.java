...
public class SimpleMarkovChainEvidence<S> implements MarkovChainEvidence<S> {
    ...
    public SimpleMarkovChainEvidence(final boolean arbitraryStart, final int order) {
        this.history = new ArrayDeque<>(order);
        this.order = order;
    ...
    private void learnFromMemory(final Collection<S> stateMemoryCollection, final S nextState) {
        final List<S> stateMemory = List.copyOf(stateMemoryCollection);
    ...
    public void learnStep(final S state) {
        ...
        if ((this.arbitraryStart) && (this.order > 1)) {
            final ArrayDeque<S> trainingMemory = new ArrayDeque<>(this.history);
    ...
    public MarkovChain<S> getMarkovChain() {
        final Map<List<S>, Map<S, Double>> transitionProbabilities = new LinkedHashMap<>(this.evidence.size());
        ...
        return new SimpleMarkovChain<>(transitionProbabilities, this.order, this.observedStates);
    }

    private static class StateCounter<S> {
        private final Map<S, Integer> stateCount = new HashMap<>();
        ...
        public Map<S, Double> probabilities() {
            final Map<S, Double> prob = new HashMap<>(this.stateCount.size());
