...
public abstract class AbstractPath<N, E extends TraversableCloud<N>> extends AbstractWalk<N, E> implements Path<N, E> {
    static <N, E extends TraversableCloud<N>> boolean verifyUtility(final List<N> nodeSteps, final List<E> edgeSteps) {
        if (nodeSteps.size() < 2)
    ...
    protected static <N, E extends TraversableCloud<N>> boolean isChain(final Path<N, E> path) {
    ...
    static <N, E extends TraversableCloud<N>> boolean isIndependentUtility(final Path<N, E> firstPath, final Path<N, E> secondPath) {
        if (!firstPath.getFirstNode().equals(secondPath.getFirstNode()))
            return false;
        if (!firstPath.getLastNode().equals(secondPath.getLastNode()))
            return false;
        final List<N> exclusiveFirstNodes = new ArrayList<>(firstPath.getNodeSteps());
