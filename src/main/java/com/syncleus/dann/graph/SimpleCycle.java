...
public class SimpleCycle<N, E extends TraversableCloud<N>> extends SimpleWalk<N, E> implements Cycle<N, E> {
    ...
    public SimpleCycle(final List<E> steps, final double defaultWeight) {
        this(steps, SimpleCycle.edgeToNodeSteps(steps), defaultWeight);
    }
    ...
    public SimpleCycle(final List<E> steps) {
        this(steps, SimpleCycle.edgeToNodeSteps(steps), 0.0);
    }
    ...
    private static <N, E extends TraversableCloud<N>> N startNodeFromSteps(final List<E> steps) {
        ...
        final List<N> exclusiveFirstNodes = new ArrayList<>(steps.get(0).getNodes());
    ...
    private static <N, E extends TraversableCloud<N>> List<N> edgeToNodeSteps(final List<E> steps) {
        if (steps == null)
        ...
        final List<N> newNodeSteps = new ArrayList<>();
        N nextNodeStep = SimpleCycle.startNodeFromSteps(steps);
        for (final E edgeStep : steps) {
            ...
            final List<N> nextNodes = new ArrayList<>(edgeStep.getNodes());
