...
public class HyperassociativeMap<G extends Graph<N, ?>, N> implements GraphDrawer<G, N> {
    ...
    public void reset() {
        resetLearning();
        // randomize all nodes
        coordinates.replaceAll((n, v) -> randomCoordinates(dimensions));
    }
    ...
    public void align() {
        ...
        if (!coordinates.keySet().equals(graph.getNodes())) {
            final Map<N, Vector> newCoordinates = new HashMap<>();
    ...
    Map<N, Double> getNeighbors(final N nodeToQuery) {
        final Map<N, Double> neighbors = new HashMap<>();
    ...
    private List<Future<Vector>> submitFutureAligns() {
        final ArrayList<Future<Vector>> futures = new ArrayList<>();
        for (final N node : graph.getNodes()) {
