...
public class JohnsonGraphTransformer<N> implements GraphTransformer<BidirectedGraph<N, ? extends WeightedDirectedEdge<N>>> {
    ...
    public BidirectedGraph<N, WeightedDirectedEdge<N>> transform(final BidirectedGraph<N, ? extends WeightedDirectedEdge<N>> original) {
        ...
        final MutableDirectedAdjacencyGraph<Object, WeightedDirectedEdge<Object>> copyGraph = new MutableDirectedAdjacencyGraph<>(new HashSet<>(original.getNodes()), originalEdges);
        final Set<Object> originalNodes = copyGraph.getNodes();
        copyGraph.add(BLANK_NODE);
        for (final Object originalNode : originalNodes)
            copyGraph.add(new ImmutableWeightedDirectedEdge<>(BLANK_NODE, originalNode, 0.0));
        final BellmanFordPathFinder<Object, WeightedDirectedEdge<Object>> pathFinder = new BellmanFordPathFinder<>(copyGraph);
        ...
        for (final WeightedDirectedEdge<N> edge : edges) {
            ...
            johnsonGraph.add(new SimpleWeightedDirectedEdge<>(edge.getSourceNode(), edge.getDestinationNode(), newWeight));
        }
