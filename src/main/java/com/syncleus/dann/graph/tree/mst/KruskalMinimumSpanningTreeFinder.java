 ...
 */
public class KruskalMinimumSpanningTreeFinder<N, E extends TraversableCloud<N>> implements MinimumSpanningTreeFinder<N, E> {
    @Override
    public Set<E> findMinimumSpanningTree(final Graph<N, E> graph) {
        final Set<Set<N>> componentNodeSets = new HashSet<>();
        for (final N node : graph.getNodes())
            componentNodeSets.add(Collections.singleton(node));
        final Queue<E> edgeQueue = new PriorityQueue<>(graph.getEdges().size(), new WeightComparator<>());
        edgeQueue.addAll(graph.getEdges());
        final Set<E> mstEdges = new HashSet<>();
