...
public final class Topography {
    ...
    public static <N, E extends TraversableCloud<N>> boolean isWeaklyConnected(final Graph<N, E> graph) {
        ...
        final List<N> remainingNodes = new ArrayList<>(graph.getNodes());
    ...
    public static <N, E extends TraversableCloud<N>> boolean isWeaklyConnected(final Graph<N, E> graph, final N leftNode, final N rightNode) {
        ...
        final Set<N> toVisit = new HashSet<>(graph.getAdjacentNodes(leftNode));
    ...
    public static <N, E extends TraversableCloud<N>> boolean isStronglyConnected(final Graph<N, E> graph, final N leftNode, final N rightNode) {
        ...
        final Set<N> visited = new HashSet<>();
        visited.add(leftNode);
        final Set<N> toVisit = new HashSet<>(graph.getTraversableNodes(leftNode));
        while (!toVisit.isEmpty()) {
            final N node = toVisit.iterator().next();
            if (node.equals(rightNode))
                return true;
    ...
    public static <N, E extends TraversableCloud<N>> boolean isMaximalSubgraph(final Graph<N, E> graph, final Graph<N, E> subGraph) {
        ...
        final Set<E> exclusiveParentEdges = new HashSet<>(graph.getEdges());
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final Set<E> cutEdges) {
        ...
        return Topography.isCut(graph, Collections.emptySet(), cutEdges);
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final N node) {
        ...
        return Topography.isCut(graph, Collections.singleton(node), Collections.emptySet());
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final E edge) {
        ...
        return Topography.isCut(graph, Collections.emptySet(), Collections.singleton(edge));
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final Set<E> cutEdges, final N begin, final N end) {
        ...
        return Topography.isCut(graph, Collections.emptySet(), cutEdges, begin, end);
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final N node, final N begin, final N end) {
        ...
        return Topography.isCut(graph, Collections.singleton(node), Collections.emptySet(), begin, end);
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isCut(final Graph<N, E> graph, final E edge, final N begin, final N end) {
        ...
        return Topography.isCut(graph, Collections.emptySet(), Collections.singleton(edge), begin, end);
    }
    ...
    public static <N, E extends TraversableCloud<N>> int getNodeConnectivity(final Graph<N, E> graph) {
        ...
        final SortedSet<Set<N>> sortedCombinations = new TreeSet<>(new SizeComparator());
        sortedCombinations.addAll(combinations);
        for (final Set<N> cutNodes : sortedCombinations)
            if (Topography.isCut(graph, cutNodes, Collections.emptySet()))
    ...
    public static <N, E extends TraversableCloud<N>> int getEdgeConnectivity(final Graph<N, E> graph) {
        ...
        final SortedSet<Set<E>> sortedCombinations = new TreeSet<>(new SizeComparator());
        sortedCombinations.addAll(combinations);
    ...
    public static <N, E extends TraversableCloud<N>> int getNodeConnectivity(final Graph<N, E> graph, final N begin, final N end) {
        ...
        sortedCombinations.addAll(combinations);
        for (final Set<N> cutNodes : sortedCombinations)
            if (Topography.isCut(graph, cutNodes, Collections.emptySet(), begin, end))
                return cutNodes.size();
        return graph.getNodes().size();
    }
    ...
    public static <N, E extends TraversableCloud<N>> boolean isMultigraph(final Graph<N, E> graph) {
        ...
        for (final N currentNode : graph.getNodes()) {
            final List<N> neighbors = new ArrayList<>(graph.getAdjacentNodes(currentNode));
            while (neighbors.remove(currentNode)) {
                //do nothing, just remove all instances of the currentNode
            }
            final Set<N> uniqueNeighbors = new HashSet<>(neighbors);
    ...
    public static <N, E extends TraversableCloud<N>> boolean isHomomorphic(final Graph<N, E> graph, final Graph<N, E> homomorphicGraph) {
        ...
        final Set<N> uncheckedNodes = new HashSet<>(graph.getNodes());
    ...
    public static <N, E extends TraversableCloud<N>> int getMultiplicity(final Graph<N, E> graph, final E edge) {
        ...
        for (final E potentialMultiple : potentialMultiples) {
            if (potentialMultiple.equals(edge))
                continue;
            final List<N> potentialNodes = new ArrayList<>(potentialMultiple.getNodes());
    ...
    private static <N, E extends TraversableCloud<N>> ImmutableAdjacencyGraph<N, TraversableCloud<N>> deleteFromGraph(final Graph<N, E> graph, final Set<N> deleteNodes, final Set<E> deleteEdges) {
        ...
        final Set<TraversableCloud<N>> cutEdges = new HashSet<>(deleteEdges);
        ...
        final Set<TraversableCloud<N>> removeEdges = new HashSet<>();
        final Set<TraversableCloud<N>> addEdges = new HashSet<>();
        for (final TraversableCloud<N> cutEdge : cutEdges) {
            ...
            if (cutEdgeNeighbors.size() > 1)
                ...
                addEdges.add(new ImmutableHyperedge<>(cutEdgeNeighbors));
        }
        ...
        // still connected
        return new ImmutableAdjacencyGraph<>(cutNodes, cutEdges);
    }
