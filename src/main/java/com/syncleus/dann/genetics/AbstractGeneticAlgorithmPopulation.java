...
public abstract class AbstractGeneticAlgorithmPopulation {
    ...
    public AbstractGeneticAlgorithmPopulation(final double ourMutationDeviation, final double ourCrossoverPercentage, final double ourDieOffPercentage) {
        this(ourMutationDeviation, ourCrossoverPercentage, ourDieOffPercentage, new ThreadPoolExecutor(Runtime.getRuntime().availableProcessors(), Runtime.getRuntime().availableProcessors() * MAX_THREADS_PER_PROCESSOR, KEEP_ALIVE_TIME, TimeUnit.SECONDS, new LinkedBlockingQueue<>()));
    }
    ...
    public final void addAll(final Collection<GeneticAlgorithmChromosome> chromosomes) {
        ...
        final List<AbstractGeneticAlgorithmFitnessFunction> initialPopulation = new ArrayList<>();
        ...
        for (final GeneticAlgorithmChromosome chromosome : chromosomes) {
            ...
            futures.add(this.threadExecutor.submit(fitnessFunction::process));
        }
    ...
    public final Set<GeneticAlgorithmChromosome> getChromosomes() {
        final Set<GeneticAlgorithmChromosome> chromosomes = new HashSet<>();
    ...
    private GeneticAlgorithmChromosome getRandomMember() {
        final int randomIndex = rng.nextInt(this.population.size());
        int currentIndex = 0;
        for (final AbstractGeneticAlgorithmFitnessFunction member : this.population) {
    ...
    public synchronized void nextGeneration() {
        ...
        while (this.population.size() < populationSize) {
            final ArrayList<GeneticAlgorithmChromosome> children = new ArrayList<>();
            while (this.population.size() + children.size() < populationSize) {
                ...
                if (rng.nextDouble() < this.crossoverPercentage) {
                    final int crossoverPoint = rng.nextInt(child1.getGenes().size() - 1) + 1;
