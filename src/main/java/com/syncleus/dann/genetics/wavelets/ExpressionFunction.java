...
public class ExpressionFunction implements Cloneable {
    ...
    public ExpressionFunction(final ExpressionFunction copy) {
        this.receptors = new HashSet<>(copy.receptors);
        this.waves = new ArrayList<>();
        for (WaveMultidimensionalFunction w : copy.waves) {
            waves.add(w.clone());
        }
    ...
    public ExpressionFunction(final ReceptorKey initialInput) {
        this.receptors = new HashSet<>(1);
        this.receptors.add(initialInput);

        this.waves = new ArrayList<>(1);
        final WaveMultidimensionalFunction initialWave = generateNewWave(ThreadLocalRandom.current());
...
//    }
     ...
     */
    public ExpressionFunction mutate(Random RANDOM/*final double deviation*/) {
        ...
        while (RANDOM.nextFloat() < changePercentage) {
            ...
            if (RANDOM.nextDouble() < changePercentage) {
                ...
                copy.waves.add(this.generateRandomWave(RANDOM));
            }
            //make a RANDOM new wave
            if (RANDOM.nextDouble() < changePercentage) {
                copy.waves.add(this.generateNewWave(RANDOM));
            }
            //delete a RANDOM wave
            if ((this.waves.size() > 1) && (RANDOM.nextDouble() < changePercentage)) {
                copy.waves.remove(copy.waves.get(RANDOM.nextInt(copy.waves.size())));
            }
            final Set<ReceptorKey> R = copy.receptors;
            final int Rn = R.size();
            //only delete if there will be at least one signal left
            if ((Rn > 1) && (RANDOM.nextDouble() < changePercentage)) {
                final ReceptorKey[] receptorArray = new ReceptorKey[Rn];
                R.toArray(receptorArray);

//                final ReceptorKey deleteReceptor = receptorArray[RANDOM.nextInt(receptorArray.length)];
//                R.remove(deleteReceptor);

                final ReceptorKey[] copyReceptors = new ReceptorKey[Rn];
                R.toArray(copyReceptors);
     ...
     * @return New mutated wavelet
     */
    public ExpressionFunction mutate(/*final double deviation, */final ReceptorKey newReceptor, Random RANDOM) {
        final ExpressionFunction copy = this.clone();
        ...
        return copy.mutate(RANDOM/*1.0*/);
    }

    private WaveMultidimensionalFunction generateNewWave(Random RANDOM) {
    ...
    private WaveMultidimensionalFunction generateRandomWave(Random RANDOM) {
