...
public class DirectedGrid extends AbstractBidirectedAdjacencyGraph<GridNode, DirectedEdge<GridNode>> {
        ...
        for (int y = 0; y < nodes.length; y++)
            for (int x = 0; x < this.nodes[0].length; x++) {
                ...
                if (x < this.nodes[0].length - 1) {
                    ImmutableDirectedEdge<GridNode> newEdge = new ImmutableDirectedEdge<>(this.nodes[y][x], this.nodes[y][x + 1]);
                    ...
                    newEdge = new ImmutableDirectedEdge<>(this.nodes[y][x + 1], this.nodes[y][x]);
                    this.edges.add(newEdge);
                ...
                if (y < nodes.length - 1) {
                    ImmutableDirectedEdge<GridNode> newEdge = new ImmutableDirectedEdge<>(this.nodes[y][x], this.nodes[y + 1][x]);
                    this.edges.add(newEdge);
                    this.outNeighborEdges.get(this.nodes[y][x]).add(newEdge);
                    ...
                    newEdge = new ImmutableDirectedEdge<>(this.nodes[y + 1][x], this.nodes[y][x]);
                    this.edges.add(newEdge);
                    this.inNeighborEdges.get(this.nodes[y][x]).add(newEdge);
    ...
    public Set<DirectedEdge<GridNode>> getAdjacentEdges(final GridNode node) {
        final Set<DirectedEdge<GridNode>> newEdges = new HashSet<>(this.inNeighborEdges.get(node));
    ...
    public Set<DirectedEdge<GridNode>> getTraversableEdges(final GridNode node) {
        final Set<DirectedEdge<GridNode>> newEdges = new HashSet<>(this.outNeighborEdges.get(node));
        return Collections.unmodifiableSet(newEdges);
    }
