    ...
    public Grid(final double[][] nodeWeights) {
        ...
        for (int y = 0; y < nodeWeights.length; y++)
            for (int x = 0; x < nodeWeights[0].length; x++) {
                ...
                this.neighborEdges.put(this.nodes[y][x], new HashSet<>());
                this.neighborNodes.put(this.nodes[y][x], new HashSet<>());
            }
        //connect nodes
        for (int y = 0; y < nodes.length; y++)
            for (int x = 0; x < this.nodes[0].length; x++) {
                ...
                if (x < this.nodes[0].length - 1) {
                    final ImmutableUndirectedEdge<GridNode> newEdge = new ImmutableUndirectedEdge<>(this.nodes[y][x], this.nodes[y][x + 1]);
                ...
                if (y < nodes.length - 1) {
                    final ImmutableUndirectedEdge<GridNode> newEdge = new ImmutableUndirectedEdge<>(this.nodes[y][x], this.nodes[y + 1][x]);
                    this.edges.add(newEdge);
                    this.neighborEdges.get(this.nodes[y][x]).add(newEdge);
