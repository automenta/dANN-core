    ...
    public SimpleUndirectedGraph(final int layers, final int nodesPerLayer) {
        ...
        for (int layerIndex = 0; layerIndex < layers; layerIndex++) {
            for (int nodeIndex = 0; nodeIndex < nodesPerLayer; nodeIndex++) {
                ...
                this.neighborEdges.put(this.nodes[layerIndex][nodeIndex], new HashSet<>());
                this.neighborNodes.put(this.nodes[layerIndex][nodeIndex], new ArrayList<>());
            }
        }
        //connect nodes
        for (int layerIndex = 0; layerIndex < (layers - 1); layerIndex++)
            for (int nodeIndex = 0; nodeIndex < nodesPerLayer; nodeIndex++) {
                for (int nodeIndex2 = 0; nodeIndex2 < nodesPerLayer; nodeIndex2++) {
                    final ImmutableUndirectedEdge<SimpleNode> newEdge = new ImmutableUndirectedEdge<>(this.nodes[layerIndex][nodeIndex], this.nodes[layerIndex + 1][nodeIndex2]);
