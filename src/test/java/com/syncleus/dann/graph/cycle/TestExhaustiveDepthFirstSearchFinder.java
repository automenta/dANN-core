...
public class TestExhaustiveDepthFirstSearchFinder {
    ...
    public void testDirectedNoCycles() {
        ...
        final DirectedEdge<Object> topRightEdge = new ImmutableDirectedEdge<>(topNode, rightNode);
        edges.add(topRightEdge);
        final DirectedEdge<Object> rightLeftEdge = new ImmutableDirectedEdge<>(rightNode, leftNode);
        edges.add(rightLeftEdge);
        final DirectedEdge<Object> topLeftEdge = new ImmutableDirectedEdge<>(topNode, leftNode);
        ...
        for (final Object cycle : finder.findCycles(graph))
            LOGGER.info(cycle);
        Assert.assertEquals("Cycles detected when there should be none: " + finder.cycleCount(graph), 0, finder.cycleCount(graph));
    }
    ...
    public void testDirectedWithCycles() {
        ...
        final Set<DirectedEdge<Object>> edges = new HashSet<>();
        final DirectedEdge<Object> bottomLeftEdge = new ImmutableDirectedEdge<>(bottomNode, leftNode);
        edges.add(bottomLeftEdge);
        final DirectedEdge<Object> letRightEdge = new ImmutableDirectedEdge<>(leftNode, rightNode);
        edges.add(letRightEdge);
        final DirectedEdge<Object> rightBottomEdge = new ImmutableDirectedEdge<>(rightNode, bottomNode);
        edges.add(rightBottomEdge);
        final DirectedEdge<Object> leftTopEdge = new ImmutableDirectedEdge<>(leftNode, topNode);
        edges.add(leftTopEdge);
        final DirectedEdge<Object> topRightEdge = new ImmutableDirectedEdge<>(topNode, rightNode);
        edges.add(topRightEdge);
        final DirectedEdge<Object> leftTippyTopEdge = new ImmutableDirectedEdge<>(leftNode, tippyTopNode);
        edges.add(leftTippyTopEdge);
        final DirectedEdge<Object> tippyTopRightEdge = new ImmutableDirectedEdge<>(tippyTopNode, rightNode);
        ...
        for (final Object cycle : finder.findCycles(graph))
            LOGGER.info(cycle);
        Assert.assertEquals("incorrect number of cycles detected. Expected 3, got: " + finder.cycleCount(graph), 3, finder.cycleCount(graph));
    }
    ...
    public void testUndirectedWithCycles() {
        ...
        final Set<BidirectedEdge<Object>> edges = new HashSet<>();
        final BidirectedEdge<Object> rightBottomEdge = new ImmutableUndirectedEdge<>(rightNode, bottomNode);
        edges.add(rightBottomEdge);
        final BidirectedEdge<Object> bottomLeftEdge = new ImmutableUndirectedEdge<>(bottomNode, leftNode);
        edges.add(bottomLeftEdge);
    ...
    public void testUndirectedWithDoubleEdgeCycles() {
        ...
        final BidirectedEdge<Object> centerRightEdge = new ImmutableUndirectedEdge<>(centerNode, rightNode);
        edges.add(centerRightEdge);
        final BidirectedEdge<Object> centerRightEdge2 = new ImmutableUndirectedEdge<>(centerNode, rightNode);
        ...
        for (final Object cycle : finder.findCycles(graph))
            LOGGER.info(cycle);
        Assert.assertEquals("incorrect number of cycles detected. Expected 1, got: " + finder.cycleCount(graph), 1, finder.cycleCount(graph));
    }
}
