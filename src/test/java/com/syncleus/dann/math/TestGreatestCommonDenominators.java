...
public class TestGreatestCommonDenominators {
    ...
    public void testBinaryGCD() {
        ...
        long gcd;
        for (final long[] solution3 : SOLUTIONS) {
            gcd = com.syncleus.dann.math.GreatestCommonDenominators.binaryGCD(firstMultiply * solution3[0], secondMultiply * solution3[1]);
            Assert.assertEquals("first:" + firstMultiply * solution3[0] + " second:" + secondMultiply * solution3[1] + " GCD:" + gcd + " expected:" + solution3[2], gcd, solution3[2]);
        }
        firstMultiply = -1;
        secondMultiply = 1;
        for (final long[] solution2 : SOLUTIONS) {
            gcd = com.syncleus.dann.math.GreatestCommonDenominators.binaryGCD(firstMultiply * solution2[0], secondMultiply * solution2[1]);
            Assert.assertEquals("first:" + firstMultiply * solution2[0] + " second:" + secondMultiply * solution2[1] + " GCD:" + gcd + " expected:" + solution2[2], gcd, solution2[2]);
        }
        firstMultiply = 1;
        secondMultiply = -1;
        for (final long[] solution1 : SOLUTIONS) {
            gcd = com.syncleus.dann.math.GreatestCommonDenominators.binaryGCD(firstMultiply * solution1[0], secondMultiply * solution1[1]);
            Assert.assertEquals("first:" + firstMultiply * solution1[0] + " second:" + secondMultiply * solution1[1] + " GCD:" + gcd + " expected:" + solution1[2], gcd, solution1[2]);
        }
        firstMultiply = -1;
        secondMultiply = -1;
        for (final long[] solution : SOLUTIONS) {
            ...
            Assert.assertEquals("first:" + firstMultiply * solution[0] + " second:" + secondMultiply * solution[1] + " GCD:" + gcd + " expected:" + solution[2], gcd, solution[2]);
        }
    }
